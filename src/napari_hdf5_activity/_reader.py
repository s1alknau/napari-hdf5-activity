"""
_reader.py - Reading recordings and detecting their structure

Domain reader for *raw recordings*: locates the image data inside an HDF5 or
Zarr store, reports its layout, and drives the chunked parallel processing used
by the analysis. Saved analysis results are a different file type and live in
:mod:`._results_io`.

Storage access goes through :mod:`._io_abstraction` wherever the operation is
format-agnostic; the remaining h5py calls implement the HDF5 branch of the
structure detection itself.

Key entry points
----------------
``detect_file_structure_type``
    Format-agnostic layout detection — use this one. ``detect_hdf5_structure_type``
    is its HDF5 branch and returns ``type="error"`` for a Zarr store.
``get_first_frame_enhanced``
    First frame as a display copy plus a processing copy.
``reader_function_dual_structure``
    The napari reader entry point.
"""
...
def preprocess_image_stack_for_processing(image_stack: np.ndarray) -> np.ndarray:
    ...
    if len(image_stack.shape) == 3:
        ...
        # Convert per frame so we never materialise a float copy of the whole
        # multi-channel stack. np.tensordot over (N, H, W, C) built an
        # N*H*W*C float32 buffer (~12 B/px) and caused MemoryErrors on large
        # colour recordings; a per-frame conversion keeps peak extra memory at
        # the single-channel output (~1 B/px for uint8). Luma weights match
        # cv2.COLOR_RGB2GRAY (ITU-R 601-2: 0.299 R + 0.587 G + 0.114 B).
        n_channels = image_stack.shape[-1]
        if n_channels in (3, 4):
            grayscale_stack = np.empty(image_stack.shape[:3], dtype=image_stack.dtype)
            try:
                import cv2

                code = cv2.COLOR_RGB2GRAY if n_channels == 3 else cv2.COLOR_RGBA2GRAY
                for i in range(image_stack.shape[0]):
                    grayscale_stack[i] = cv2.cvtColor(image_stack[i], code)
            except Exception:
                # Fallback without a full multi-channel float copy: accumulate
                # the luma of one frame at a time.
                for i in range(image_stack.shape[0]):
                    f = image_stack[i, :, :, 0].astype(np.float32) * 0.299
                    f += image_stack[i, :, :, 1].astype(np.float32) * 0.587
                    f += image_stack[i, :, :, 2].astype(np.float32) * 0.114
                    grayscale_stack[i] = f.astype(image_stack.dtype)
        else:
            logger.warning(f"Unexpected number of channels: {n_channels}")
            grayscale_stack = image_stack[:, :, :, 0].copy()  # Take first channel

        logger.info(
            f"Converted colour stack {image_stack.shape} to grayscale "
            f"{grayscale_stack.shape} (per-frame, low-memory)"
        )
...
def detect_circles_and_create_masks(
    ...
    try:
        ...
        # Reduce whatever we get to a single 2D uint8 grayscale image.
        # cv2.HoughCircles / CLAHE require CV_8UC1, so this covers colour frames,
        # frame stacks, and non-8-bit cameras (e.g. 12-bit data stored as uint16).
        gray_frame = frame
        ...
        if gray_frame.ndim == 3 and gray_frame.shape[-1] in (3, 4):  # colour -> gray
            ...
            gray_frame = gray_frame[0]
        if gray_frame.dtype != np.uint8:  # 12-bit uint16 / float -> normalise to 8-bit
            ...
            logger.info(f"Normalized detection frame ({frame.dtype}) to uint8 for HoughCircles")
...
def process_chunk(
    ...
    try:
        ...
        # Normalize by the storage dtype maximum (uint8 -> 255, uint16 -> 65535),
        # matching get_frame_norm_factor() (which keys on the camera bit depth via
        # dtype itemsize). Determined here, before the int16 cast discards the
        # original dtype. The downstream pipeline (hysteresis, fraction-of-movement,
        # circadian analysis) is scale-invariant, so this factor sets the units of
        # the stored Delta-I but does not change any detection result.
        norm_factor = 65535.0 if chunk_data.dtype.itemsize >= 2 else 255.0
        ...
        # Division by the dtype maximum is deferred to the final scalar divide.
        ...
        for roi_idx, mask in enumerate(masks, start=1):
            try:
                ...
                # Stored value is the per-pixel mean of the dtype-normalized diff,
                # i.e. sum / (n_pixels * norm_factor), which lies in [0,1].
                # The GUI "÷ ROI pixel count" checkbox shows this as-is (per-pixel mode)
                # or multiplies by n_pixels * norm_factor for the MATLAB-equivalent pixel sum.
                roi_pixels = frames_xp[:, mask_bool]                                                 # (N, n_pixels) int16
                diffs      = xp.abs(xp.diff(roi_pixels, axis=0))                                     # (N-1, n_pixels) int16
                roi_intensities_xp = diffs.sum(axis=1, dtype=xp.float32) / (n_pixels * norm_factor)  # (N-1,) in [0,1]
...
def get_roi_colors(rois: List[int]) -> Dict[int, str]:
    ...
    In batch mode the keys are composite ids (dataset_idx * 1000 + roi_id), so
    the colour is picked from the within-dataset ROI number: ROI 1 of every
    dataset shares one colour and the dataset is distinguished by its label.
    """
    from ._batch import base_roi_id
    ...
    for roi_id in rois:
        ...
        roi_colors[roi_id] = color_cycle[(base_roi_id(roi_id) - 1) % len(color_cycle)]
