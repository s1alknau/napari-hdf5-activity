    ...
    try:
        ...
        with h5py.File(file_path, "r", **(dict(driver=driver) if driver else {})) as f:
            ...
            if structure_info["type"] == "stacked_frames":
                ...
                logger.info(
                    f"Loaded first frame from stacked dataset: {first_frame.shape}"
                )
                ...
                logger.info(
                    f"Loaded first frame from images group: key={first_key}, shape={first_frame.shape}"
                )
                ...
                logger.info(
                    f"Loaded first frame from alternative dataset '{dataset_name}': {first_frame.shape}"
                )
...
def reader_directory_function(
    ...
    for filename in filenames[:10]:  # Limit previews to prevent memory issues
        try:
            preview_frame, preview_structure = get_first_frame_enhanced(
                filename, driver
            )
    ...
    try:
        ...
        if len(chunk_data.shape) == 4 and chunk_data.shape[-1] == 3:
            ...
            for i in range(0, len(chunk_data), batch_size):
                ...
                if CV2_AVAILABLE:
                    gray_batch = np.array(
                        [cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY) for frame in batch]
                    )
                else:
...
def _process_single_chunk_dual_structure(
    ...
    try:
        ...
        chunk = read_chunk_data_dual_structure(
            file_path, start_idx, end_idx, structure_info
        )
        ...
        if first_frame is None:
            ...
            if len(first_frame.shape) == 3:
                ...
                labeled_frame = (
                    ...
                    if CV2_AVAILABLE
                    else np.stack([first_frame] * 3, axis=-1)
                )
